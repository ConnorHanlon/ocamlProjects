...
module Lazee_v1 : LazeeSig = struct
  ...
  let force (l: 'a t) : unit = match !l with
    | Value _ -> ()
    | Thunk f -> l := Value (f ())	      
  let delay (u:unit -> 'a): 'a t = ref(Thunk u)
...
module Lazee_v2 : LazeeSig = struct
  ...
  (*modify so that v1 implements lazeesig following the def of lazy values from lazy.ml

-may define values not in lazeesig signature and thus not used outside of lazee_v1
  *)
