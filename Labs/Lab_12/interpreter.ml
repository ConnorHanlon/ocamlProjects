...
let rec exec (s: stmt) (stt: state) : state =
  ...
  | Skip -> stt
  | For (s, lower_expr, upper_expr, body_stmt) ->
     let lower = eval lower_expr [] in
     let equals = Lt (upper_expr, lower_expr) in
     (match eval equals  stt with
     | Bool false ->
	exec (For (s, Add (Val (Int 1), lower_expr), upper_expr, body_stmt)) (exec body_stmt ((s, lower)::stt))
     | Bool true -> stt
     | _ -> raise (Failure "Incompatible types on For")
     )
       
let my_mod_tester = Mod (Val (Int 4), Val (Int 2))
...
(*Review Section
ran exec program_while [];;
10
My answer:
sum appears 11 times.
*)
let num_sum = 11	
...
(*Code below is by Professor Eric Van Wyk*)
